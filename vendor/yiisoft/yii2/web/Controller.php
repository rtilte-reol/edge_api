...
class Controller extends \yii\base\Controller
{
    ...
    public function bindActionParams($action, $params)
    {
        ...
        // Accept parameters with dashes in their key name (e.g. access-token will be interpreted as accessToken)
        foreach($params as $key => $value) {
            if(strpos($key, "-") !== false) {
                $newKey = $key;
                while(strpos($newKey,"-") !== false) {
                    $pos = strpos($newKey,"-");
                    $newKey = substr($newKey, 0, $pos) . ucfirst(substr($newKey, $pos + 1));
                }
                $params[$newKey] = $value;
            }
        }
